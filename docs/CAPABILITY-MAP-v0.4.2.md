# Capability Map · @toolated/coherence-mcp v0.4.2

**Audience:** Claude (or any strand) cross-check + smoke test  
**Package:** `@toolated/coherence-mcp@0.4.2`  
**Invariant:** α + ω = 15  
...
**Known drift for Claude to flag:** `Server({ version: "0.3.2" })` in `src/index.ts` does not match package `0.4.2`. Smoke test should record this; fix is a patch commit.
...
## 4. Out-of-band (not in 0.4.2 tarball tools list)
...
```bash
npm view @toolated/coherence-mcp version   # 0.4.2
node -v                                    # >= 18
npx -y @toolated/coherence-mcp@0.4.2 --help  # or start under MCP client
```
...
3. ATOM: `atom_track` with decision string `SMOKE-0.4.2`  
...
```
CAPABILITY SMOKE · @toolated/coherence-mcp@0.4.2
date:
client:
...
invariant: PASS|FAIL
wave_sample_score:
known_drift: server.version 0.3.2 vs package 0.4.2
...
- [ ] `npm list -g @toolated/coherence-mcp` or local install shows **0.4.2**  
...
- [RELEASE_v0.4.2.md](RELEASE_v0.4.2.md) — release narrative  
